# HISTOS retrieval benchmarks

Status: **H00 candidate / deterministic local tooling only**

The real public v1 suite has independently frozen gold and exact token profiles.
See [`../docs/CONTEXT_CONTRACT_V1.md`](../docs/CONTEXT_CONTRACT_V1.md) for source,
capsule, freeze and measurement contracts. The older synthetic v0 byte scaffold
below remains a separate regression surface.
...
npm test
npm run bench:frozen
node tools/bench/frozen-benchmark.mjs bench/public-gold-v1.json bench/public-gold-freeze-v1.json 2k
```

This runs a fixed lexical baseline against three exact public `46slv/chatgpt-mcp`
modules from commit `d353159c6a9e7622603819b608422169c9a1f9d4`. It contains three
development and four held-out cases, including a missing-source negative. All
seven were source-reviewed by a separate verifier before scoring. The freeze
receipt binds source manifest and gold by SHA-256; changes are refused.

V1 uses **2000 / 4000 / 8000 tokens**, counted by exact-pinned
`js-tiktoken@1.0.21` with `cl100k_base`. Each profile includes all of the
`rendered_context` preamble and source headers in its token budget. Audit-envelope
JSON is not model input. This repository-local dependency is required for v1;
the v0 runners below still use Node built-ins alone. No paid API or model runs.

Read the measured failures as well as recall: keyword overlap does not establish
answerability, and partial overlap does not guarantee complete evidence. Reports
show overlap recall, complete-range recall, fragment and line precision,
rendered tokens/bytes, raw source exposure and direct scan/read counts. Missing
cache, stale task-outcome and model-outcome dimensions say `NOT_MEASURED` or
`NOT_APPLICABLE` with a reason. Whole-file exposure is a counterfactual rendering
of the same loaded files, not another IO or agent experiment.

Frozen source and gold files use `-text` Git attributes to preserve byte hashes
across Windows checkouts. Original source is never executed by the benchmark.

The untuned baseline with portable source fingerprints is retained in
[`evidence/public-baseline-v1-portable.json`](evidence/public-baseline-v1-portable.json), with a
concise [result and limitations](evidence/public-baseline-v1.md). It has known
incomplete-evidence and no-answer failures; freezing the benchmark does not
mean that retrieval quality passed a production acceptance gate.
